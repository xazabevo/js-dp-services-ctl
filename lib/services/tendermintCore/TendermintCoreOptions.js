...
class TendermintCoreOptions extends DockerServiceOptions {
  ...
  mergeWithDefaultOptions(...customOptions) {
    ...
    const defaultServiceOptions = {
      port: customOptions[0].port || defaultPorts.port,
    };

    const abciUrl = customOptions[0].abciUrl || 'tcp://127.0.0.1:26658';

    const cmd = ['node', `--rpc.laddr=tcp://0.0.0.0:${defaultServiceOptions.port}`, `--proxy_app=${abciUrl}`];

    const defaultContainerOptions = {
      image: 'tendermint/tendermint:v0.31.11',
      network: {
